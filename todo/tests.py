...
class TodoViewTestCase(TestCase):
    ...
    def test_detail_get_sfail(self):
        client = Client()
    ...
    def test_update_post_success(self):
        task = Task(title='before', due_at=timezone.make_aware(datetime(2024, 7, 1, 0, 0, 0)))
        task.save()

        client = Client()
        data = {'title': 'after', 'due_at': '2024-07-31 12:34:56'}
        ...
        task = Task.objects.get(pk=task.pk)
        self.assertEqual(task.title, 'after')
        self.assertEqual(task.due_at, timezone.make_aware(datetime(2024, 7, 31, 12, 34, 56)))
